...
initmax = (np.pi -0.01)
initres = 20 # resolution of measured points, for square array only
...
def calculate_fractal(t_max, t_1_init, t_2_init, verbose, valueprint):
    fractal = [] # array definitions outside the loops, so they can be used after the loop is done   
    for l in range(1, len(t_2_init)): # begin to loop over t_2-Values
        ...
        for m in range(1, len(t_1_init)): #initialisation of the row's value loop, loop over t_1 values with const t_2
            breakvalue = fliptime(t_max, t_1_init[m], t_2_init[l], m, l, verbose, valueprint)
            fractal_row.append(breakvalue) # appending breakvalue to row 
...
#fractal= logization(calculate_fractal(t_max, t_1_init, t_2_init, True, False))
fractal = calculate_fractal(t_max, t_1_init, t_2_init, True, False)
fig = plt.figure()
plt.pcolormesh(fractal)
plt.xlabel(("Initial Value Resolution=", initres , "Time Depth=", t_max))
#plt.tight_layout()
plt.show()
