...
t_1_init = np.linspace(initmin, initmax, initres)
t_2_init = np.linspace(initmin, initmax, initres) # here obviously t_1_init could be used, but, if other shapes of arrays should be needed it's easier to build the code directly for two dims instead of a double use of one dim.
