...
def kinetic(q1, q2, p1, p2):
	return 1/(2*m*l**2) * (p1**2+2*p2**2-2*p1*p2*np.cos(q1-q2))/(1+(np.sin(q1-q2))**2)

def potential(q1,q2):
	return m*g*l*(-2*np.cos(q1)-np.cos(q2))
...
for i in range(1,n):
	p1new = p1 - dt/(2*m*l**2)*((2*p1*p2*np.sin(q1-q2))*(1+(np.sin(q1-q2))**2)-2*np.sin(q1-q2)*np.cos(q1-q2)*(p1**2+2*p2**2-2*p1*p2*np.cos(q1-q2)))/((1+(np.sin(q1-q2))**2)**2) + 2*m*g*l*np.sin(q1)
	p2new = p2 + dt/(2*m*l**2)*((2*p1*p2*np.sin(q1-q2))*(1+(np.sin(q1-q2))**2)-2*np.sin(q1-q2)*np.cos(q1-q2)*(p1**2+2*p2**2-2*p1*p2*np.cos(q1-q2)))/((1+(np.sin(q1-q2))**2)**2) + m*g*l*np.sin(q2)

	q1new = q1 + dt/(m*l**2*(1+(np.sin(q1-q2))**2))*(p1new-p2new*np.cos(q1-q2))
	q2new = q2 + dt/(m*l**2*(1+(np.sin(q1-q2))**2))*(2*p2new-p1new*np.cos(q1-q2))
...
functions.saveDataToFile("leapfrog_test.txt", t, q1a, q2a, p1a, p2a, m, m, l, l, g, tf, dt, 4)