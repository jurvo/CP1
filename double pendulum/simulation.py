...
def simulation(t_1_0, t_2_0, v_1_0, v_2_0, m_1, m_2, l_1, l_2, g, t_max, delta_t, simulation_mode, verbose, filename):
	if verbose:
	...
	if verbose:
		...
		print("Saving...")
	saveDataToFile(filename, t[:t_break], t_1[:t_break], t_2[:t_break], v_1[:t_break], v_2[:t_break], m_1, m_2, l_1, l_2, g, t_max, delta_t, simulation_mode)
...
for dt in dts:
	temp = simulation(t_1_0, t_2_0, v_1_0, v_2_0, m_1, m_2, l_1, l_2, g, t_max, dt, simulation_mode, verbose, "dt" + str(dt) + ".txt")
	tbreak.append(temp)

fig = plt.figure()
plt.loglog(dts, tbreak)
plt.show()